...
func checkUTLSFingerPrintClientHello(t *testing.T, clientHelloID ClientHelloID, serverName string) {
	uconn := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, clientHelloID, false, false, nil)
	if err := uconn.BuildHandshakeState(); err != nil {
		t.Errorf("got error: %v; expected to succeed", err)
	}

	generatedUConn := UClient(&net.TCPConn{}, &Config{ServerName: "foobar"}, HelloCustom, false, false, nil)
...
func TestUTLSFingerprintClientHelloBluntMimicry(t *testing.T) {
	...
	uconn := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, HelloCustom, false, false, nil)
...
func TestUTLSFingerprintClientHelloAlwaysAddPadding(t *testing.T) {
	...
	uconnWithoutPadding := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, HelloCustom, false, false, nil)
	uconnWithPadding := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, HelloCustom, false, false, nil)
...
func TestUTLSHandshakeClientFingerprintedSpecFromChrome_58(t *testing.T) {
	...
	uconn := UClient(&net.TCPConn{}, originalConfig, helloID, false, false, nil)
	if err := uconn.BuildHandshakeState(); err != nil {
		t.Errorf("got error: %v; expected to succeed", err)
	}
